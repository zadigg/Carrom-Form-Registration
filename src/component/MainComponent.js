...
const MainComponent = () => {
    ...
    return (
        <div className="bg-white dark:bg-gray-900 flex">
            <div className="hidden lg:block lg:w-2/3 bg-cover" style={{ backgroundImage: `url(${ProjectJailBreakImageThree})` }}>
            </div>
            <div className="w-full max-w-md px-6 mx-auto lg:w-1/3 mt-14">
                <div className="bg-gray-50 px-5 py-3 dark:bg-gray-800 flex justify-between">
                    <button onClick={() => handleViewChange('Register')}
                            className={view === 'Register' ? 'text-blue-50' : 'text-gray-500'}>Register
                    </button>
                    <button onClick={() => handleViewChange('participants')}
                            className={view === 'participants' ? 'text-blue-50' : 'text-gray-500'}>Participants
                    </button>
                </div>
                <div
                    className={`flex flex-col ${view === 'Register' ? 'justify-center' : 'justify-start'} mx-auto overflow-auto`}
